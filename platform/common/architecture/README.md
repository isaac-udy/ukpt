...
- The module's plain `test` task runs nothing — the suite is generated by the architecture Gradle plugin into its own `architectureTest` source set (no test sources are checked in).
