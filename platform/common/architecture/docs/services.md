...
> Sources: @Describe annotations in the Kotlin catalog in `src/main/kotlin/architecture/rules/services/` (narrative + rules), plus the `*.examples.md` files beside it.
...
* A declaration placed in `services.tools` may depend on the Service contract via `:api`-defined types only — never on `services.storage` or `services.internal`
...
* A Codec encapsulates the read/write asymmetry `setFromRow` can't express — it stays small and keyed to the column it serves
