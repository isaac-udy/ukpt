...
* **Definition**: The orchestrators that compose subsystems (e.g. `SessionProcessingManager`) — see the [`services.internal` overview](#servicesinternal). Cross-subsystem composition belongs here, at bare `services.internal`, not to imports between sibling subsystems.

* **Construct** `ServicesLayer.InternalCoordinator` (`🔶 construct`) — a declaration is this construct when it satisfies all of:
    * resides in `feature..services..`
...
* **Definition**: Payloads that flow from one subsystem through the orchestrator into another. A carrier lives at the bare `services.internal` ancestor so both producer and consumer can name it under the data-shape carve-out (see [hierarchical visibility](#hierarchical-visibility-within-servicesinternal)).

* **Construct** `ServicesLayer.InternalDataCarrier` (`🔶 construct`) — a declaration is this construct when it satisfies all of:
    * resides in `feature..services..`
...
* **Definition**: Abstractions used inside a subsystem (e.g. a strategy contract whose implementations live in the same subpackage).

* **Construct** `ServicesLayer.InternalInterface` (`🔶 construct`) — a declaration is this construct when it satisfies all of:
    * resides in `feature..services..`
    * is an interface
    * Resides in `feature.[name].services.internal`
...
* **Definition**: Exceptions thrown only by internal helpers; service-level exceptions belong on the `Service` interface (see [Services](#services-the-cross-the-wire-contract)).

* **Construct** `ServicesLayer.InternalException` (`🔶 construct`) — a declaration is this construct when it satisfies all of:
    * resides in `feature..services..`
...
* **Construct** `ServicesLayer.InternalObjectHelper` (`🔶 construct`) — a declaration is this construct when it satisfies all of:
    * resides in `feature..services..`
    * is an object
    * Resides in `feature.[name].services.internal`
...
* **Definition**: The hand-written persistence record shapes — the `XxxRow`/`XxxRecord`/`XxxInsert` `data class`es that live in a feature's `services.storage`. The *generated* `XxxRow` classes live in `platform.server.postgres.tables` instead — see [generated `Table`/`Row` sources](#generated-tablerow-sources).

* **Construct** `ServicesLayer.StorageRecord` (`🔶 construct`) — a declaration is this construct when it satisfies all of:
    * resides in `feature..services..`
    * Is a `data class`
...
* **Construct** `ServicesLayer.MappingFunction` (`🔶 construct`) — a declaration is this construct when it satisfies all of:
    * resides in `feature..services..`
    * is a function
    * Resides in `feature.[name].services.storage`
* **Rules**:
