...
The complete catalog, one row per construct or rule. Ids are object/property paths (see the [README](../README.md)). `✅ tested` = executable check · `🔶 construct` = classification requirements · `📋 guidance` = documented convention · `⚙️ codegen` = delegated to code generation.
