...
* **Codegen:** `exportPostgresSchema` Flyway-migrates a throwaway Zonky Postgres and writes a
  ...
  `compileKotlin`. Generated files live under `build/generated/source/postgres-tables/` and
  are not committed.
* **Runtime ownership:** the DB primitives (`PostgresConfig`, `PostgresMigrator`,
  ...
  starts serving.
* **Dev database:** `:app:server:run` provides a local Postgres selected by `UKPT_DEV_DB`; the
  modes and seeding switches are documented in the `ukpt-run` skill. Seed data lives in
  `:platform:server:development` as `DevScenario` objects listed in `DevScenarios`, each
  writing the generated `Table` objects directly in one `suspendTransaction`. A scenario is
  fixture data, not persistence code — the module is not part of any feature's `server.data`,
  it is the only module that may depend on the Zonky binaries, and the fat jar excludes both.
