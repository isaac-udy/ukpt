...
import architecture.registry.Exemptions
import architecture.registry.ModuleGraph
import architecture.registry.ModuleGraphConstraint
import architecture.registry.parse
...
import architecture.registry.sourceLocation
...
/** The canonical rule index: every enforced rule as `id | enforcement | statement`, in document order. */
internal fun renderRuleIndex(groups: List<RuleGroup>): String = buildString {
    appendLine("| Rule | Enforcement | Statement |")
    appendLine("| --- | --- | --- |")
    enforcedRules(groups)
        .filter { it.status is Status.Active }
        .forEach { rule -> appendLine("| `${rule.id}` | ${rule.tag.marker} | ${rule.title.replace("|", "\\|")} |") }
}.trimEnd()
