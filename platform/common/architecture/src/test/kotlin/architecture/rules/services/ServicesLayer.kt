...
object ServicesLayer : RuleGroup(
    ...
    @Describe("A declaration placed in `services.tools` may depend on the Service contract via `:api`-defined types only — never on `services.storage` or `services.internal`")
