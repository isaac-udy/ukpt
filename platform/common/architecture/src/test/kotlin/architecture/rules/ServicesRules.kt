 ...
 * group level.
 *
 * Rule ids are the `by val` path, e.g. `servicesLayer.storageClass.returnsRowTypesOnly`.
 */
val servicesLayer by rules(inPackage = "feature..services..") {

    // ---- §4.4.1 Services (the cross-the-wire contract, `:api`) --------------------------------
    val serviceInterface by construct {
        // what it is
        val urpcInterface by requirement("A service is an `interface` annotated `@Urpc`") {
            iface { decl -> decl.annotations.any { it.name == "Urpc" } }
        }
        val nameEndsWithService by requirement("Is named `[Name]Service`") {
            hasNameEndingWith("Service")
        }
        val inServicesContractPackage by requirement("Resides in the top-level `feature.[name].services` package") {
            any { it.isInServicesRoot() }
        }
    ...
    val serviceImpl by construct {
        val serviceImplClass by requirement("For a service named `[Name]Service` the implementation is a class named `[Name]ServiceImpl`") {
            cls { it.name.endsWith("ServiceImpl") }
        }
        val internalVisibility by requirement("Service implementations must be `internal`") {
            hasModifier(KoModifier.INTERNAL)
        }
        val inServerServicesPackage by requirement("Resides in `feature.[name].services` of the `:server` module (dual-life with the contract)") {
            any { it.isInServicesRoot() }
        }
    ...
    val internalCoordinator by construct {
        val concreteCoordinatorClass by requirement("A coordinator is a concrete (non-`abstract`, non-`data`) class that is not a `Job` or `Exception`") {
            cls { decl ->
        ...
        val inInternalPackage by requirement("Resides in `feature.[name].services.internal`") {
            any { it.isInServicesSubAxis("internal") }
        }
    }

    val internalDataCarrier by construct {
        val dataCarrierClass by requirement("A data carrier is a `data class` payload that flows between subsystems through the orchestrator") {
    ...
    val internalInterface by construct {
        val internalInterfaceShape by requirement("An internal interface is an abstraction used inside a subsystem") {
            isInterface()
        }
        val inInternalPackage by requirement("Resides in `feature.[name].services.internal`") {
            any { it.isInServicesSubAxis("internal") }
    ...
    val internalException by construct {
        val internalExceptionClass by requirement("An internal exception is a class named `[Name]Exception`, thrown only by internal helpers") {
            cls { it.name.endsWith("Exception") }
        }
        val inInternalPackage by requirement("Resides in `feature.[name].services.internal`") {
            any { it.isInServicesSubAxis("internal") }
        }
    ...
    val internalObjectHelper by construct {
        val objectHelperShape by requirement("An object helper is an `object` holding pure helpers") {
            isObject()
        }
        val inInternalPackage by requirement("Resides in `feature.[name].services.internal`") {
            any { it.isInServicesSubAxis("internal") }
    ...
    val storageClass by construct {
        val storageNameSuffix by requirement("Named `[Name]Storage` (or `[Name]Store` where the broader name fits)") {
            cls { it.name.endsWith("Storage") || it.name.endsWith("Store") }
        }
        val notAbstractNotData by requirement("Not abstract, not a `data class`") {
            cls { !it.hasAbstractModifier && !it.hasDataModifier }
        }
        val internalVisibility by requirement("`internal` visibility") {
            hasModifier(KoModifier.INTERNAL)
        }
        val inStoragePackage by requirement("Resides in `feature.[name].services.storage`") {
            any { it.isInServicesSubAxis("storage") }
        }
    ...
    val storageRecord by construct {
        val rowDataClass by requirement("Is a `data class`") {
            cls { it.hasDataModifier }
        }
        val rowNameSuffix by requirement("Named `[Name]Row`, `[Name]Record`, or `[Name]Insert`") {
            hasNameEndingWith("Row").or(hasNameEndingWith("Record")).or(hasNameEndingWith("Insert"))
        }
        val inStoragePackage by requirement("Resides in `feature.[name].services.storage`") {
            any { it.isInServicesSubAxis("storage") }
        }
    ...
    val mappingFunction by construct {
        val topLevelFunction by requirement("A mapping function is a top-level function converting between a `XxxRow` and a domain type") {
            isFunction()
        }
        val inStoragePackage by requirement("Resides in `feature.[name].services.storage`") {
            any { it.isInServicesSubAxis("storage") }
    ...
    val codecObject by construct {
        val codecObjectShape by requirement("A codec is an `object` (discriminator constants, or file-private `Json` + encode/decode helpers)") {
            isObject()
        }
        val inStoragePackage by requirement("Lives in `services.storage` alongside the Row + mapping functions for the table that uses it") {
            any { it.isInServicesSubAxis("storage") }
        }
