...
- The README and everything under `platform/common/architecture/docs/` are **generated**: narrative comes from `.md` fragment files next to each layer's rules (`rules/data/DataLayer.md` for the layer, `rules/data/DataLayer.Repository.md` for one construct), and the generator compiles them into a fixed per-layer structure with the rule content from the catalog. Edit a fragment or the catalog — never the generated files — then regenerate (see Testing).
